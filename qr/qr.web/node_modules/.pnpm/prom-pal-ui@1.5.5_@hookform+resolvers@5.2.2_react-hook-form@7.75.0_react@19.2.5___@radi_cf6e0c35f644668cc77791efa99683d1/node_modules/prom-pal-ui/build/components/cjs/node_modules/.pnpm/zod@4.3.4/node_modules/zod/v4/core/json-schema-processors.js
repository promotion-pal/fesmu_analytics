...
var toJsonSchema = require('./to-json-schema.js');
...
const arrayProcessor = (schema, ctx, _json, params) => {
    ...
    json.type = "array";
    json.items = toJsonSchema.process(def.element, ctx, { ...params, path: [...params.path, "items"] });
};
...
const tupleProcessor = (schema, ctx, _json, params) => {
    ...
    const prefixItems = def.items.map((x, i) => toJsonSchema.process(x, ctx, {
        ...params,
        path: [...params.path, prefixPath, i],
    }));
    const rest = def.rest
        ? toJsonSchema.process(def.rest, ctx, {
            ...params,
...
const recordProcessor = (schema, ctx, _json, params) => {
    ...
    if (def.mode === "loose" && patterns && patterns.size > 0) {
        ...
        const valueSchema = toJsonSchema.process(def.valueType, ctx, {
            ...params,
        ...
        if (ctx.target === "draft-07" || ctx.target === "draft-2020-12") {
            json.propertyNames = toJsonSchema.process(def.keyType, ctx, {
                ...params,
                path: [...params.path, "propertyNames"],
            });
        }
        json.additionalProperties = toJsonSchema.process(def.valueType, ctx, {
            ...params,
            path: [...params.path, "additionalProperties"],
        });
    }
...
function toJSONSchema(input, params) {
    if ("_idmap" in input) {
        ...
        const ctx = toJsonSchema.initializeContext({ ...params, processors: allProcessors });
        const defs = {};
        // First pass: process all schemas to build the seen map
        for (const entry of registry._idmap.entries()) {
            const [_, schema] = entry;
            toJsonSchema.process(schema, ctx);
        }
        const schemas = {};
        const external = {
            registry,
        ...
        for (const entry of registry._idmap.entries()) {
            ...
            toJsonSchema.extractDefs(ctx, schema);
            schemas[key] = toJsonSchema.finalize(ctx, schema);
        }
    ...
    // Single schema case
    const ctx = toJsonSchema.initializeContext({ ...params, processors: allProcessors });
    toJsonSchema.process(input, ctx);
    toJsonSchema.extractDefs(ctx, input);
    return toJsonSchema.finalize(ctx, input);
}

exports.allProcessors = allProcessors;
