...
export function useAnalytic(): UseAnalytic {
  ...
  const [rawToilets, setRawToilets] = useState<ToiletResDto[]>([]); // Храним исходные данные
  ...
  const filterRatingsByDate = (ratings: any[], filter: TimeFilter) => {
    ...
    return ratings.filter((rating) => {
      ...
      if (filter.from && ratingDate < filter.from) return false;
      if (filter.to && ratingDate > filter.to) return false;
      return true;
    });
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const filtered = applyFilterToToilets(toiletsData, timeFilter);
      setToilets(filtered);
  ...
  // Применяем фильтр при его изменении
  const handleSetTimeFilter = useCallback(
    (filter: TimeFilter) => {
      setTimeFilter(filter);

      // Фильтруем исходные данные, а не уже отфильтрованные
      if (rawToilets.length > 0) {
        const filtered = applyFilterToToilets(rawToilets, filter);
        setToilets(filtered);
        calculateStatsFromToilets(filtered);
      }
    },
...
//     (rawData: ToiletResDto[], filter: TimeFilter) => {
//       const filtered = applyFilterToToilets(rawData, filter);
//       setToilets(filtered);
//       calculateStatsFromToilets(filtered);
//     },
//     [],
...
//     try {
//       const toiletsData = await api.toiletsControllerGetAllToilets();
//       updateData(toiletsData, timeFilter);
//     } catch (err) {
//       setError("Ошибка при загрузке данных");
//       console.error("Error fetching data:", err);
//     } finally {
//       setLoading(false);
//     }
//   }, [api, timeFilter, updateData]);

//   const handleSetTimeFilter = useCallback((filter: TimeFilter) => {
//     setTimeFilter(filter);
//     setToilets((currentToilets) => {
//       const filtered = applyFilterToToilets(currentToilets, filter);
//       calculateStatsFromToilets(filtered);
//       return filtered;
//     });
//   }, []);
